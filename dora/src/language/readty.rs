...
fn read_type_basic_unchecked(
    ...
    match sym {
        ...
        Some(Sym::Value(value_id)) => {
            ...
            if let Some(ref primitive_ty) = value.primitive_ty {
...
fn verify_type_basic(
    ...
    match ty {
        ...
        | SourceType::Float64 => {
            let value_id = ty
                .primitive_value_id(sa)
                .expect("primitive struct expected");

            if !value_accessible_from(sa, value_id, module_id) {
                let value = sa.values.idx(value_id);
                let value = value.read();
                let msg = ErrorMessage::NotAccessible(value.name(sa));
                sa.diag.lock().report(file_id, node.pos, msg);
                return false;
            }
        }
        ...
        SourceType::Union(union_id, type_params) => {
            let union = sa.unions.idx(union_id);
            let union = union.read();

            if !union_accessible_from(sa, union_id, module_id) {
                let msg = ErrorMessage::NotAccessible(union.name(sa));
                sa.diag.lock().report(file_id, node.pos, msg);
                return false;
            }
    ...
    F: FnOnce(&TypeParamDefinition) -> R,
{
    match ctxt {
        ...
        TypeParamContext::Value(value_id) => {
            let value = &sa.values.idx(value_id);
            let value = value.read();
        ...
        TypeParamContext::Union(union_id) => {
            let union = &sa.unions[union_id];
            let union = union.read();

            callback(union.type_params())
        }
