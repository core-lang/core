...
thread_local! {
    static THREAD: RefCell<*const CoreThread> = RefCell::new(ptr::null());
}

pub fn current_thread() -> &'static CoreThread {
    let thread = THREAD.with(|thread| *thread.borrow());
    debug_assert!(!thread.is_null());
    unsafe { &*thread }
}

pub fn init_current_thread(thread: Arc<CoreThread>) -> &'static CoreThread {
...
pub struct Threads {
    pub threads: Mutex<Vec<Arc<CoreThread>>>,
    pub cv_join: Condvar,
...
impl Threads {
    ...
    pub fn add_thread(&self, thread: Arc<CoreThread>) {
    ...
    pub fn add_main_thread(&self, thread: Arc<CoreThread>) {
...
pub struct CoreThreadPtr(Address);

impl CoreThreadPtr {
    pub fn null() -> CoreThreadPtr {
        CoreThreadPtr(Address::null())
    }

    pub fn new(thread: &CoreThread) -> CoreThreadPtr {
        CoreThreadPtr(Address::from_ptr(thread as *const _))
    }

    pub fn to_ref(self) -> &'static CoreThread {
        unsafe { &*self.0.to_ptr::<CoreThread>() }
    }
...
unsafe impl Sync for CoreThread {}
unsafe impl Send for CoreThread {}

impl CoreThread {
    pub fn new(vm: &VM, initial_state: ThreadState) -> Arc<CoreThread> {
        CoreThread::with_id(vm.threads.next_thread_id(), initial_state)
    }

    fn with_id(id: usize, initial_state: ThreadState) -> Arc<CoreThread> {
        Arc::new(CoreThread {
    ...
    pub fn dtn(&self) -> *const CoreToNativeInfo {
        self.tld.dtn.load(Ordering::Relaxed) as *const _
    }

    pub fn set_dtn(&self, ptr: *const CoreToNativeInfo) {
        self.tld.dtn.store(ptr as usize, Ordering::Relaxed);
    }

    pub fn use_dtn<F, R>(&self, dtn: &mut CoreToNativeInfo, fct: F) -> R
    where
        F: FnOnce() -> R,
    {
        dtn.last = self.dtn();
    ...
    pub fn push_dtn(&self, dtn: &mut CoreToNativeInfo) {
        dtn.last = self.dtn();
        self.set_dtn(dtn as *const _);
    }
    ...
    pub fn prepare_for_waitlist(&self) {
        ...
        *data = (true, CoreThreadPtr::null());
    }

    pub fn set_waitlist_successor(&self, new_tail: CoreThreadPtr) {
        let mut data = self.blocking_data.blocking.lock();
        let (blocking, next) = *data;
        assert!(blocking && next.is_null());
        *data = (true, new_tail);
    ...
    pub fn remove_from_waitlist(&self) -> CoreThreadPtr {
        let next = {
            let mut thread_data = self.blocking_data.blocking.lock();
            let (blocking, next) = *thread_data;
            assert!(blocking);
            *thread_data = (false, CoreThreadPtr::null());
            next
        };
...
struct BlockingData {
    blocking: Mutex<(bool, CoreThreadPtr)>,
    cv_blocking: Condvar,
}
...
impl BlockingData {
    fn new() -> BlockingData {
        BlockingData {
            blocking: Mutex::new((false, CoreThreadPtr::null())),
...
impl ManagedThread {
    ...
    pub fn install_native_thread(&mut self, native_thread: &Arc<CoreThread>) {
        self.native_thread_ptr = Arc::as_ptr(native_thread) as usize as u64;
        self.id = native_thread.id() as u64;
    }

    pub fn native_thread(&self) -> &'static CoreThread {
