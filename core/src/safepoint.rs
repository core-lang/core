...
use crate::stdlib;
use crate::threads::{current_thread, parked_scope, CoreThread, ThreadState};
use crate::vm::{get_vm, Trap, VM};

pub fn stop_the_world<F, R>(vm: &VM, operation: F) -> R
where
    F: FnOnce(&[Arc<CoreThread>]) -> R,
{
    parked_scope(|| {
...
fn stop_threads(vm: &VM, threads: &[Arc<CoreThread>]) {
...
fn resume_threads(vm: &VM, threads: &[Arc<CoreThread>]) {
    for thread in threads.iter() {
