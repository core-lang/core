...
#[repr(C)]
pub struct CoreToNativeInfo {
    // pointer to previous DTN-Info
    pub last: *const CoreToNativeInfo,
...
impl CoreToNativeInfo {
    pub fn new() -> CoreToNativeInfo {
        CoreToNativeInfo {
            last: ptr::null(),
            fp: 0,
            pc: 0,
    ...
    pub fn last_offset() -> i32 {
        offset_of!(CoreToNativeInfo, last) as i32
    }

    pub fn fp_offset() -> i32 {
        offset_of!(CoreToNativeInfo, fp) as i32
    }

    pub fn pc_offset() -> i32 {
        offset_of!(CoreToNativeInfo, pc) as i32
    }
}
