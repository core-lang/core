...
impl<'x> ValueCheck<'x> {
    fn check(&mut self) {
        ...
        {
            let value = self.sa.values.idx(self.value_id);
            let value = value.read();

            for (id, name) in value.type_params().names() {
                self.symtable.insert(name, Sym::TypeParam(id));
            }
        }

        for (idx, field) in self.ast.fields.iter().enumerate() {
            self.visit_value_field(field, idx.into());
        }

        self.symtable.pop_level();
    }

    fn visit_value_field(&mut self, f: &ast::ValueField, id: ValueDefinitionFieldId) {
        ...
        let old = value.field_names.insert(f.name, id);
        assert!(old.is_none());
    }
}
