...
use core_parser::ast::{self, visit};
use core_parser::interner::Name;
use core_parser::parser::Parser;
...
impl<'a> ProgramParser<'a> {
    ...
    fn add_stdlib_package(&mut self) {
        ...
        if let Some(stdlib) = self.packages.remove(stdlib_name) {
            ...
            let stdlib_file = format!("stdlib{}stdlib.core", std::path::MAIN_SEPARATOR);
    ...
    fn add_module_files(
        ...
        match file_lookup {
            FileLookup::FileSystem => {
                ...
                file_path.push(format!("{}.core", name));
