...
use crate::compiler;
use crate::compiler::core_exit_stubs::NativeStubs;
...
use crate::threads::{
    current_thread, deinit_current_thread, init_current_thread, CoreThread, ThreadState, Threads,
    STACK_SIZE,
};
use crate::utils::GrowableVecNonIter;
use crate::utils::{GrowableVec, MutableVec};

use core_parser::interner::*;
...
pub use self::specialize::{
    add_ref_fields, specialize_class_id, specialize_class_id_params, specialize_enum_class,
    specialize_enum_id_params, specialize_lambda, specialize_trait_object, specialize_tuple_array,
...
impl VM {
    ...
    pub fn run(&self, fct_id: FctDefinitionId) -> i32 {
        ...
        let core_stub_address = self.stubs.core_entry();
        let fct: extern "C" fn(Address, Address) -> i32 =
            unsafe { mem::transmute(core_stub_address) };
        fct(tld, ptr)
    }
    ...
    pub fn ensure_compiled(&self, fct_id: FctDefinitionId) -> Address {
        let mut dtn = CoreToNativeInfo::new();
    ...
    F: FnOnce() -> R,
{
    ...
    let native_thread = CoreThread::new(vm, ThreadState::Running);
