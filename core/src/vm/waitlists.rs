...
use crate::threads::{current_thread, CoreThreadPtr};
use crate::vm::get_vm;
...
impl WaitLists {
    ...
    pub fn block(&self, mutex: Handle<ManagedMutex>, expected_value: i32) {
        ...
        let thread_ptr = CoreThreadPtr::new(thread);
    ...
    fn conditionally_enqueue<F>(&self, thread_ptr: CoreThreadPtr, object: Address, fct: F) -> bool
    where
        F: FnOnce() -> bool,
    {
...
impl Default for HeadAndTail {
    fn default() -> HeadAndTail {
        HeadAndTail {
            head: CoreThreadPtr::null(),
            tail: CoreThreadPtr::null(),
        }
    }
}
