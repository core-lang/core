...
use crate::threads::CoreThread;
use crate::vm::{
    get_concrete_tuple_ty, specialize_enum_id_params, value_instance, CodeKind, EnumLayout, VM,
};

pub fn determine_strong_roots(vm: &VM, threads: &[Arc<CoreThread>]) -> Vec<Slot> {
...
pub fn iterate_strong_roots<F: FnMut(Slot)>(vm: &VM, threads: &[Arc<CoreThread>], mut callback: F) {
...
fn iterate_roots_from_handles<F: FnMut(Slot)>(thread: &CoreThread, callback: &mut F) {
...
fn iterate_roots_from_stack<F: FnMut(Slot)>(vm: &VM, thread: &CoreThread, callback: &mut F) {
    let mut dtn = thread.dtn();

    while !dtn.is_null() {
        dtn = iterate_roots_from_core_to_native_info(vm, dtn, callback);
    }
}

fn iterate_roots_from_core_to_native_info<F: FnMut(Slot)>(
    vm: &VM,
    dtn: *const CoreToNativeInfo,
    callback: &mut F,
) -> *const CoreToNativeInfo {
    let dtn = unsafe { &*dtn };
