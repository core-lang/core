...
struct CoreEntryGen<'a> {
    vm: &'a VM,
    masm: MacroAssembler,
    dbg: bool,
}

impl<'a> CoreEntryGen<'a> {
    pub fn install(self) -> Arc<Code> {
        let vm = self.vm;
        let code_descriptor = CoreEntryGen::generate(self);
        install_code_stub(vm, code_descriptor, CodeKind::CoreStub)
    }
