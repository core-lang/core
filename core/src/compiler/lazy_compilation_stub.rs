...
// of this function be used in Core. It is necessary for lazy compilation, where
...
struct CoreCompileGen<'a> {
    vm: &'a VM,
    masm: MacroAssembler,
    dbg: bool,
}

impl<'a> CoreCompileGen<'a> {
    pub fn generate(mut self) -> Arc<Code> {
        ...
        let offset_params = offset_dtn + size_of::<CoreToNativeInfo>() as i32;
        let offset_thread =
        ...
        self.masm.store_mem(
            MachineMode::Ptr,
            Mem::Base(REG_SP, offset_dtn + CoreToNativeInfo::last_offset()),
            REG_TMP1.into(),
        );

        self.masm.store_mem(
            MachineMode::Ptr,
            Mem::Base(REG_SP, offset_dtn + CoreToNativeInfo::fp_offset()),
            REG_FP.into(),
        );
        ...
        self.masm.store_mem(
            MachineMode::Ptr,
            Mem::Base(REG_SP, offset_dtn + CoreToNativeInfo::pc_offset()),
            REG_TMP1.into(),
        );
